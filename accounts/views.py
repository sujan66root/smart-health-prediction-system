...
from main_app.models import doctorr, doctor
...
def signup_doctorr(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['username'] and request.POST['email'] and request.POST['name'] and request.POST['dob'] and request.POST['gender'] and request.POST['address'] and request.POST['mobile'] and request.POST['password'] and request.POST['password1']:
            ...
            if password == password1:
                if User.objects.filter(username=username).exists():
                    ...
                    return redirect('signup_doctorr')
                ...
                return redirect('sign_in_doctorr')
        ...
        return render(request, 'doctorr/signup_Form/signup.html')


def sign_in_doctorr(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            try:
                if (user.doctorr.is_patient == True):
                    auth.login(request, user)

                    request.session['doctorrusername'] = user.username

                    return redirect('doctorr_ui')
        ...
        return render(request, 'doctorr/signin_page/index.html')


def savepdata(request, doctorrusername):
    ...
    if request.method == 'POST':
        ...
        doctorr.objects.filter(pk=puser.doctorr).update(
            name=name, dob=dobdate, gender=gender, address=address, mobile_no=mobile_no)

        return redirect('pviewprofile', doctorrusername)
