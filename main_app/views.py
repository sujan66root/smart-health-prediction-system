...
def admin_ui(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'doctorr/doctorr_ui/profile.html')


def doctorr_ui(request):

    if request.method == 'GET':

        if request.user.is_authenticated:

            doctorrusername = request.session['doctorrusername']
            puser = User.objects.get(username=doctorrusername)

            return render(request, 'doctorr/doctorr_ui/profile.html', {"puser": puser})
...
def pviewprofile(request, doctorrusername):

    if request.method == 'GET':

        puser = User.objects.get(username=doctorrusername)

        return render(request, 'doctorr/view_profile/view_profile.html', {"puser": puser})
...
def checkdisease(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'doctorr/checkdisease/checkdisease.html', {"list2": alphabaticsymptomslist})
        ...
        """
            ...
            doctorr = puser.doctorr
            diseasename = predicted_disease
            no_of_symp = inputno
            symptomsname = psymptoms
            confidence = confidencescore

            diseaseinfo_new = diseaseinfo(patient=doctorr, diseasename=diseasename, no_of_symp=no_of_symp,
